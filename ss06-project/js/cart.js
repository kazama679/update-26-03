    ...
    for (let i = 0; i < userLogin.length; i++) {
        ...
        if(userLogin[i].email==checkLogin){
            ...
            if(userLogin[i].cart ==""){
                ...
                <a href="../index.html" class="btn-cart">Mua ngay</a>
                </div>
            `
            } else{
                for (let j = 0; j < userLogin[i].cart.length; j++) {
                    ...
                    <td class="product-name">${userLogin[i].cart[j].name}</td>
                    <td class="text-price">${formatter.format(userLogin[i].cart[j].price)}</td>
                    <td><button class="change-count-product" onclick="reduceProduct(${userLogin[i].cart[j].id})">-</button>
                    <input class="quantityProduct" type="text" value="${userLogin[i].cart[j].quantity}" style="width: 40px"><button class="change-count-product" onclick="increaseProduct(${userLogin[i].cart[j].id})">+</button></td>
                    <td class="text-price">${formatter.format(userLogin[i].cart[j].quantity*userLogin[i].cart[j].price)}</td>
